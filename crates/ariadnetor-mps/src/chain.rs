//! Tensor chain accessor traits — common operations for MPS / MPO.
//!
//! - [`TensorChainRepr`] reads sites as `R: TensorRepr` (backs
//!   [`MpsRepr`] / [`MpoRepr`]).
...
use arnet_tensor::{Storage, StorageFor, TensorData, TensorLayout, TensorRepr};

use super::types::{CanonicalForm, Mpo, MpoRepr, Mps, MpsRepr};

// ============================================================================
// `R: TensorRepr` trait — backs `MpsRepr` / `MpoRepr`
// ============================================================================

/// Common operations for MPS / MPO tensor chains whose sites are
/// `R: TensorRepr` (i.e. backing [`MpsRepr`] / [`MpoRepr`]).
pub trait TensorChainRepr<R: TensorRepr, B: ComputeBackend> {
    /// Number of sites.
    fn len(&self) -> usize;
...
macro_rules! impl_tensor_chain_repr {
    ($type:ident) => {
        impl<R: TensorRepr, B: ComputeBackend> TensorChainRepr<R, B> for $type<R, B> {
            fn len(&self) -> usize {
                self.0.storages.len()
            }
...
impl_tensor_chain_repr!(MpsRepr);
impl_tensor_chain_repr!(MpoRepr);
