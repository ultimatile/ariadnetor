    ...
    C: TensorChain<DenseStorage<T>, DenseLayout, B>,
{
    let (q_tensor, r) = {
        ...
        // order.
        let q_rm = reorder_dense_tensor(&q, order, MemoryOrder::RowMajor);
        let k = q_rm.shape()[1];
        let mut q_shape = orig_shape[..rank - 1].to_vec();
        q_shape.push(k);
        let q_multi = dense_reshape(&q_rm, q_shape);
        let q_back = reorder_dense_tensor(&q_multi, MemoryOrder::RowMajor, order);
    ...
    B: ComputeBackend,
{
    ...
    let next_rm = reorder_dense_tensor(next, order, MemoryOrder::RowMajor);
    ...
    let next_2d = reorder_dense_tensor(&next_2d_rm, MemoryOrder::RowMajor, order);
    let result_2d = contract(r, &next_2d, "ab,bc->ac")
        .expect("R absorption into next site failed during canonicalize");

    let result_2d_rm = reorder_dense_tensor(&result_2d, order, MemoryOrder::RowMajor);
    let k = r.shape()[0];
    let mut new_shape = next_shape;
    new_shape[0] = k;
    let result_multi = dense_reshape(&result_2d_rm, new_shape);
    reorder_dense_tensor(&result_multi, MemoryOrder::RowMajor, order)
}
    ...
    B: ComputeBackend,
{
    ...
    let prev_rm = reorder_dense_tensor(prev, order, MemoryOrder::RowMajor);
    ...
    let prev_2d = reorder_dense_tensor(&prev_2d_rm, MemoryOrder::RowMajor, order);
