...
//! [`BlockSparse::dagger`] (which flips QN directions, conjugates
...
//!
//! [`BlockSparse::dagger`]: arnet_tensor::BlockSparse::dagger
    ...
    S: Sector,
{
    ...
    let mut out = BlockSparse::<T, S>::zeros(new_indices, t.flux().clone());
...
) -> Result<BlockSparse<T, S>, DmrgEnvError>
where
    T: Scalar,
    S: Sector,
{
    ...
    let mut env = BlockSparse::<T, S>::zeros(vec![env_leg0, env_leg1, env_leg2], S::identity());
...
impl<T, S> DmrgEnvOps for BlockSparse<T, S>
where
    T: Scalar,
    S: Sector,
{
