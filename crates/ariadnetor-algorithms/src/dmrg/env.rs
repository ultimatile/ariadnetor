...
//! W-bond, bot-ket-bond)` matching the convention used by
//! `arnet_mps::inner::braket_dense`. Boundary slots (`left[0]` and
//! `right[N]`) are pure 1×1×1 identities.
//!
...
#[derive(Debug, Clone)]
pub struct DmrgEnvs<T: Scalar, B: ComputeBackend = NativeBackend> {
    /// `left[i]` for `i in 0..=n_sites`. `left[0]` is the trivial
    /// 1×1×1 boundary; `left[i]` for `i > 0` carries sites `0..i`.
    /// `None` indicates the slot is stale relative to the current
    /// sweep position.
    left: Vec<Option<Dense<T>>>,
    /// Mirror of `left` for the right sweep. `right[N]` is the
    /// trivial boundary; `right[j]` for `j < N` carries sites
    /// `j..N`.
    right: Vec<Option<Dense<T>>>,
    n_sites: usize,
    backend: Arc<B>,
}

impl<T: Scalar, B: ComputeBackend> DmrgEnvs<T, B> {
    ...
    pub fn build(mps: &Mps<Dense<T>, B>, mpo: &Mpo<Dense<T>, B>) -> Result<Self, DmrgEnvError> {
        let n_sites = mps.len();
        if n_sites == 0 {
        ...
        let mut left: Vec<Option<Dense<T>>> = vec![None; n_sites + 1];
        let mut right: Vec<Option<Dense<T>>> = vec![None; n_sites + 1];

        // Trivial 1×1×1 boundary tensors at the chain edges.
        left[0] = Some(trivial_boundary(&*backend));
        right[n_sites] = Some(trivial_boundary(&*backend));
        ...
        for j in (1..=n_sites).rev() {
            ...
            let new = extend_right_step(&*backend, prev, mps.storage(j - 1), mpo.storage(j - 1))?;
            right[j - 1] = Some(new);
        }
    ...
    pub fn advance_left(
        ...
        let new = extend_left_step(&*self.backend, prev, mps.storage(i), mpo.storage(i))?;
    ...
    pub fn advance_right(
        ...
        let new = extend_right_step(&*self.backend, prev, mps.storage(j), mpo.storage(j))?;
...
/// Build the trivial 1×1×1 boundary tensor (single element with value 1).
fn trivial_boundary<T: Scalar, B: ComputeBackend>(backend: &B) -> Dense<T> {
    backend.make_tensor(vec![T::one()], vec![1, 1, 1])
}

/// Per-site left extension: `(top-bra, W, bot-ket)` env, single-MPS bra=ket.
///
/// Mirrors `arnet_mps::inner::braket_dense`'s loop body. Output env
/// has the same `(top, W, bot)` axis order as the input, advanced by
/// one site to the right.
fn extend_left_step<T: Scalar, B: ComputeBackend>(
    backend: &B,
    env: &Dense<T>,
    site: &Dense<T>,
    mpo_site: &Dense<T>,
...
/// Per-site right extension: mirror of [`extend_left_step`]. Output
/// has the same `(top, W, bot)` axis order as `extend_left_step`'s
/// output, advanced by one site to the left.
fn extend_right_step<T: Scalar, B: ComputeBackend>(
    backend: &B,
    env: &Dense<T>,
    site: &Dense<T>,
    mpo_site: &Dense<T>,
