...
use crate::dmrg::LocalEigensolverParams;
...
fn bsp_heff_matvec_matches_dense_oracle() {
    ...
    for (case, v) in test_inputs.iter().enumerate() {
        let v_dense_flat = Host::shared().dense(v.clone(), vec![dim]);
...
fn bsp_heff_matvec_matches_dense_oracle_n3_bulk() {
    ...
    // `BraketEnvs::build` populates `left[0]` (boundary) and
    ...
    for (case, v) in test_inputs.iter().enumerate() {
        let bsp_out = bsp_heff.apply(&Host::shared().dense(v.clone(), vec![dim]));
        let psi_dense = build_dense_psi_from_flat(v, &template);
...
fn bsp_heff_step_eigenvalue_matches_eigh_on_bsp_flat() {
    ...
    let h_dense = Host::shared().dense(h_data, vec![dim, dim]);
