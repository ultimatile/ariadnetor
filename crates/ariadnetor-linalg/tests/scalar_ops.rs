...
fn test_scale_f64() {
    ...
    // `[1, 2, 3, 4]`); assertions read via `DenseTensorData::get` which uses
...
fn test_scale_complex() {
    ...
    let tensor = DenseTensorData::from_raw_parts(
    ...
    // (1+0i)*(2+3i) = 2+3i
    assert_eq!(scaled.data()[0], Complex::new(2.0, 3.0));
    // (0+1i)*(2+3i) = -3+2i
    assert_eq!(scaled.data()[1], Complex::new(-3.0, 2.0));
}
...
fn test_scale_column_major() {
    ...
    // Scaled flat: [2, 6, 4, 8]
    assert_eq!(scaled.data(), &[2.0, 6.0, 4.0, 8.0]);
}

// test_scale_non_contiguous removed: Dense no longer supports non-contiguous views
...
fn test_normalize_zero_panics() {
    let tensor = DenseTensorData::<f64>::zeros(vec![2, 2]);
...
fn test_normalize_column_major() {
    ...
    assert!((normalized.data()[0] - 1.0 * expected_scale).abs() < 1e-10);
    assert!((normalized.data()[1] - 3.0 * expected_scale).abs() < 1e-10);
}
...
fn test_linear_combine_shape_mismatch() {
    let a = DenseTensorData::<f64>::constant(vec![2, 2], 1.0);
    let b = DenseTensorData::<f64>::constant(vec![3, 3], 2.0);
...
fn test_trace_matrix() {
    ...
    let mat = DenseTensorData::<f64>::from_raw_parts(
        vec![1.0, 2.0, 3.0, 4.0],
        vec![2, 2],
        MemoryOrder::RowMajor,
    );
...
fn test_trace_3x3_identity() {
    ...
    data[4] = 1.0;
    data[8] = 1.0;
    let mat = DenseTensorData::from_raw_parts(data, vec![3, 3], MemoryOrder::ColumnMajor);
    let result = trace(&mat, &[(0, 1)]).unwrap();
...
fn test_trace_partial_rank3() {
    ...
    data[13] = 5.0; // [1,1,1]
    data[17] = 6.0; // [1,2,2]
    let tensor = DenseTensorData::from_raw_parts(data, vec![2, 3, 3], MemoryOrder::RowMajor);
    let result = trace(&tensor, &[(1, 2)]).unwrap();
    assert_eq!(result.shape(), &[2]);
    assert_eq!(result.data()[0], 6.0);
    assert_eq!(result.data()[1], 15.0);
}
...
fn test_trace_tci_example() {
    ...
    let tensor = DenseTensorData::from_raw_parts(data, shape, MemoryOrder::RowMajor);
...
fn test_trace_dimension_mismatch() {
    let tensor =
        DenseTensorData::from_raw_parts(vec![0.0; 6], vec![2, 3], MemoryOrder::ColumnMajor);
...
fn test_trace_index_out_of_range() {
    let tensor =
        DenseTensorData::from_raw_parts(vec![0.0; 4], vec![2, 2], MemoryOrder::ColumnMajor);
...
fn test_trace_duplicate_index() {
    let tensor =
        DenseTensorData::from_raw_parts(vec![0.0; 8], vec![2, 2, 2], MemoryOrder::ColumnMajor);
...
fn test_diag_construct_3x3() {
    let v = DenseTensorData::<f64>::from_raw_parts(
        vec![2.0, 5.0, 8.0],
        vec![3],
        MemoryOrder::ColumnMajor,
    );
...
fn test_diag_identity() {
    let id = DenseTensorData::<f64>::eye(3);
...
fn test_diag_round_trip() {
    let v =
        DenseTensorData::<f64>::from_raw_parts(vec![3.0, 7.0], vec![2], MemoryOrder::ColumnMajor);
    let m = diag(&v).unwrap();
...
fn test_diag_complex() {
    ...
    let v = DenseTensorData::from_raw_parts(
...
fn test_diag_rank3_error() {
    let a = DenseTensorData::from_raw_parts(vec![0.0; 8], vec![2, 2, 2], MemoryOrder::ColumnMajor);
    assert!(diag(&a).is_err());
}
