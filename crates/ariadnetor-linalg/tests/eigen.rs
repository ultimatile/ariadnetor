...
fn test_eigh_f64_3x3_diagonal() {
    ...
    assert_eq!(w.shape(), &[3]);
    assert!((w.data_slice()[0] - 1.0).abs() < 1e-10);
    assert!((w.data_slice()[1] - 2.0).abs() < 1e-10);
    assert!((w.data_slice()[2] - 3.0).abs() < 1e-10);
}
...
fn test_eigh_c64_hermitian() {
    ...
    let w0: f64 = w.data_slice()[0];
    let w1: f64 = w.data_slice()[1];
...
fn test_eigh_f32() {
    ...
    assert!((w.data_slice()[1] - 3.0).abs() < 1e-5);
}
...
fn test_eig_c64_complex_input() {
    ...
    let mut eigs: Vec<Complex<f64>> = (0..2).map(|i| w.data_slice()[i]).collect();
