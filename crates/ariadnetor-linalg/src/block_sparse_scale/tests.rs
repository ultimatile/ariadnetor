...
fn scale_vt_axis0() {
    ...
    let svt = diagonal_scale_block_sparse(&vt, &sv, 0).unwrap();
...
fn scale_u_axis_last() {
    ...
    let us = diagonal_scale_block_sparse(&u, &sv, u.rank() - 1).unwrap();
...
fn scale_sqrt_via_map() {
    ...
    let u_scaled = diagonal_scale_block_sparse(&u, &sqrt_sv, u.rank() - 1).unwrap();
    let vt_scaled = diagonal_scale_block_sparse(&vt, &sqrt_sv, 0).unwrap();
...
fn scale_identity_weights() {
    ...
    let vt_scaled = diagonal_scale_block_sparse(&vt, &ones, 0).unwrap();
...
fn scale_axis_out_of_range() {
    ...
    let result = diagonal_scale_block_sparse(&bs, &weights, 5);
