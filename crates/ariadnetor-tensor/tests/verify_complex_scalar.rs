...
fn test_complex_scale_real_in_normalize() {
    ...
    // This is exactly what happens in normalize
}
...
fn test_norm_returns_real_type() {
    ...
    let complex_tensor = RawTensor::from_data(complex_data, vec![1]);

    let norm: f64 = complex_tensor.norm(); // Should be f64, not Complex<f64>
    assert_eq!(norm, 5.0);

    // This would not compile if norm returned Complex<f64>:
    // let _: f64 = norm; // ✓ OK
}
...
fn test_generic_function_with_scalar() {
    ...
    let real_tensor = RawTensor::<f64>::ones(vec![2, 2]);
