...
fn test_replace_slice_column_major() {
    ...
    let mut t = Dense::<f64>::new(vec![0.0; 16], vec![4, 4]);
    let sub = Dense::<f64>::new(vec![1.0, 3.0, 2.0, 4.0], vec![2, 2]);
    t.replace_slice(&sub, &[1, 1], MemoryOrder::ColumnMajor);
...
fn test_replace_slice_cow() {
    ...
    let mut a = Dense::<f64>::new(vec![0.0; 4], vec![2, 2]);
    let b = a.clone();
    let sub = Dense::<f64>::new(vec![1.0], vec![1, 1]);
    a.replace_slice(&sub, &[0, 0], MemoryOrder::RowMajor);
