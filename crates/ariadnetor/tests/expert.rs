//! Umbrella-surface test for the `ariadnetor::expert` per-call-policy layer.
//!
//! Verifies the expert layer is fully expressible from `ariadnetor` alone: the five
...
//! consumer needs no direct `ariadnetor-core` / `ariadnetor-linalg` dependency to pin a
//! policy. Naming every path through `ariadnetor::` is the load-bearing part: each
...
fn expert_layer_reachable_through_umbrella() {
    ...
    // via `ariadnetor::ExecPolicy`. The shape assertion is a structural smoke test;
